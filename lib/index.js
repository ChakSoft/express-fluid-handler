...
const Fn = require('./fn')
const pipeStream = require('./stream')
      ...
      if (!intermediate) {
        if (typeof result === 'object') {
          if (result instanceof Stream) {
            if (res.get('content-range')) {
              res.status(req.mustUseStatus || 206) // If range, so it's partial content
            }
            return await pipeStream(result, res)
          }
